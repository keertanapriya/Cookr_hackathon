import sys
#from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTextBrowser, QLineEdit, QDialog
...
# Step 2: Cleaning, Tokenization, Lowercasing
...
class CookrInterface(QMainWindow):
    ...
    def show_recipes(self):
        ...
        # Add user input widget only under "Recipes" section
        for dish in self.previous_dishes:
            dish_input = QLineEdit()
            dish_input.setReadOnly(True)
            dish_input.setText(dish)
            input_layout.addWidget(dish_input)
        
        # Add a new input box for the next dish
        ...
        # Fetch the first 10 recipes from the dataset
        data = pd.read_csv('cookr_food_data.csv')
        recipes = data['name'].tolist()[:10]
        
        # Set up the HTML content to display the recipes
        ...
        # Set the QTextBrowser content
        self.text_browser.setStyleSheet("font-size: 20px; background-image: url('image_3.jpg'); background-position: center; background-repeat: no-repeat; background-size: cover;")
    ...
    def add_new_recipe(self, new_dish_input):
        ...
        if len(words) == 1:
            ...
            with open('cookr_food_data.csv', 'a', newline='') as f:
                attributes = [
                ...
                new_row = {
                    ...
                    'taste': None,
                    'time_of_taking': None,
                    'cuisine_followed': None,
                    'region': None,
                    'protein_rich_indicator': None,
                    'nutritional_benefits': None
                }

                for i, feature_value in enumerate(new_dish_features):
                    if feature_value == 1:
                        attribute_index = i % len(attributes)
                        new_row[attributes[attribute_index]] = data[attributes[attribute_index]].unique()[1]
                    else:
                        attribute_index = i % len(attributes)
                        new_row[attributes[attribute_index]] = data[attributes[attribute_index]].unique()[0]
